      ...
      <div className="container-x relative py-20">
          ...
          <nav className="flex flex-wrap gap-x-8 gap-y-3">
            {NAV_ITEMS.map((item) => (
              <a
                ...
                className="text-sm tracking-tight text-white/60 transition-colors hover:text-sand"
              >
                {item.label}
              </a>
            ))}
        ...
        <div className="mt-12 flex flex-col gap-8 md:flex-row md:items-end md:justify-between">
          ...
          <div className="flex flex-col gap-6 md:flex-row md:gap-16">
            {email && (
              <div>
                <p className="text-xs uppercase tracking-[0.15em] text-white/35">Kontakt</p>
                <a
                  href={email.href ?? '#'}
                  className="mt-1 inline-block text-sm text-white/70 transition-colors hover:text-sand"
                >
                  {email.value}
                </a>
              </div>
